...
Implementado siguiendo principios SOLID con configuración externa y logging detallado.
"""
...
class ColumnValidator(IValidator):
    ...
    def validar(self, archivo: str, contenido: str, logger: MigrationLogger) -> Tuple[bool, Optional[str]]:
        ...
        if tipo_archivo is None:
            # CAMBIO: Ahora rechaza archivos no parametrizados
        ...
        try:
            ...
            if tiene_comodin:
                ...
                if diferencias:
                    ...
                    # Informar sobre columnas adicionales (no es error con comodín)
                    if cantidad_encontrada > cantidad_obligatorias:
                        logger.log(
...
class SmartFileHandler(IFileHandler):
    ...
    def leer(self, ruta: str, logger: MigrationLogger) -> str:
        ...
        encodings_a_probar = [
            'utf-8-sig', 'utf-8', 'windows-1252', 'cp1252',
            'iso-8859-1', 'latin-1', 'cp850', 'cp437'
        ]
        ...
        # Fallback
        logger.log(
            f"[WARN] Encoding desconocido, usando windows-1252 con reemplazos")
        try:
